...
- stateless : 통신이 끝나면 상태를 유지하지 않는 성질(서버가 클라이언트 식별X)
...
- 쿠키
  ...
  - **클라이언트(브라우저)**단에서 관리되는 기록 파일
  - 동작
    - 클라이언트 측에서 요청
    - 서버에서 쿠키 생성 후 응답
    - 클라이언트에서 요청시 **HTTP 헤더에 쿠키를 포함**시켜 전송
  - 특징
    - 유효시간 존재
    - 보안에 취약 -> 중간에서 탈취 가능 -> 대안 : 세션
...
- 세션
  ...
  - 동작
    ...
    - 클라이언트에서 세션 ID에 대해 쿠키를 사용해서 저장(JSESSIONID)
    - 클라이언트 측에서 서버에 접속시 쿠키를 통해 세션 ID를 서버에 전달 

  - 특징
    - 클라이언트마다 고유 **세션 ID** 부여
    - 클라이언트(브라우저)가 종료되거나 유효시간 만료 될 때 까지 상태 유지
    - 보안 면에서 쿠키보다 우수
    - 사실 세션도 중간에서 탈취 가능하다... -> 대안 필요
